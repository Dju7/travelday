  ...
  return (
    <section>
      ...
      <div className={`h-[40%] w-full ${countryData ? '' : 'bg-destination'} bg-cover bg-center rounded-xl flex flex-col gap-4 justify-center items-center`} style={countryData ? { backgroundImage: `url(${countryData.urlImage})` } : {}}>
            ...
            <input className='w-[40%] h-12 p-2 bg-white bg-opacity-60 border-2 border-white' onChange={(e) => setCountry(e.target.value)}/>
            <button className='p-1 w-[10%] h-10 bg-blue-600 text-xl font-bold text-white rounded-xl' onClick={getCountries}>Voir</button>
